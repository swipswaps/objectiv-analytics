...
import { NonEmptyArray, TrackerConsole, TrackerTransportInterface, TransportableEvent } from '@objectiv/tracker-core';
...
export class DebugTransport implements TrackerTransportInterface {
  ...
  async handle(...args: NonEmptyArray<TransportableEvent>): Promise<any> {
    ...
    (await Promise.all(args)).forEach((trackerEvent) => TrackerConsole.debug(JSON.parse(JSON.stringify(trackerEvent))));
  }
