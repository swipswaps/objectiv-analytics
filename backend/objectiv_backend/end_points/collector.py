...
def write_sync_events(ok_events: EventDataList, nok_events: EventDataList, event_errors: List[EventError] = None):
    ...
    if output_config.snowplow:
        write_data_to_snowplow_if_configured(events=ok_events, prefix='OK')
        write_data_to_snowplow_if_configured(events=nok_events, prefix='NOK', event_errors=event_errors)
