...
from objectiv_backend.workers.worker_enrichment import main_enrichment
...
def call_all(loop: bool):
    while True:
        ...
        event_count += worker_main(function=main_enrichment, loop=False)
...
def main():
    ...
    parser.add_argument('type',
                        choices=['all', 'entry', 'enrichment', 'finalize'],
                        default='all',
                        type=str)
    ...
    if args.type == 'enrichment':
        return worker_main(function=main_enrichment, loop=args.loop)
